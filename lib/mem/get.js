...
const last_parent = function(doc,keys,force) {
  ...
  var
    nested = doc,//nested can be Array or Object as we iterate over the keys
    depth = 0
  while(depth < (keys.length -1)) {
    ...
    if (!nested || typeof nested !== 'object') throw new Error(
      'No nested Object found for key at depth ' + depth + ' for key ' + keys[depth] + ' in  ' + JSON.stringify(keys)+ ' , for the top Object ' + JSON.stringify(doc) + ' found instead ' + JSON.stringify(nested)
    )
    depth += 1
  }
  return nested
