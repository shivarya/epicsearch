...
var delete_dups = function(params) {
  ...
  es = this.es,
  fields = [source_key]//We want only the source_key in objects sent in response
  ...
  .then(function(dup_vals){
    ...
    if(dup_vals.length) {
      ...
      return es.get_first({key:key, val:dup_vals, index: index, type: type, sort: sort, fields:fields}) //These first docs will not be deleted, all except them will be deleted. Hence sort is important 
...
if(require.main === module){
  ...
  es.find_delete_dups({key:"url",del_sort:{fetch_time:"desc"},index:"test",type:"test"}).
  then(function(res){
    debug(res)
  })
}
